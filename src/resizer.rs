...
impl ImageResizer {
    ...
    fn calculate_dimensions(&self, image: &DynamicImage, mode: ResizeMode) -> (u32, u32) {
        ...
        match mode {
            ...
            ResizeMode::Scale(scale) => {
                ...
                let new_width = (orig_width as f32 * scale / 100.0) as u32;
                let new_height = (orig_height as f32 * scale / 100.0) as u32;
            ...
            ResizeMode::Width(width) => {
                ...
                let height = (orig_height as f32 * ratio) as u32;
                (width, height.max(1))
            }
            ResizeMode::Height(height) => {
                ...
                let width = (orig_width as f32 * ratio) as u32;
                (width.max(1), height)
            }
        }
    }
    ...
    fn preserve_aspect(&self, orig_w: u32, orig_h: u32, target_w: u32, target_h: u32) -> (u32, u32) {
        ...
        if target_w == 0 {
            ...
            let width = (orig_w as f32 * ratio) as u32;
            return (width.max(1), target_h);
        }
        
        if target_h == 0 {
            let ratio = target_w as f32 / orig_w as f32;
            let height = (orig_h as f32 * ratio) as u32;
    ...
    pub fn calculate_mode_from_config(width: u32, height: u32, scale: f32) -> ResizeMode {
        if scale > 0.0 {
            ...
            ResizeMode::Absolute(0, 0) // No resize
        }
    }
}